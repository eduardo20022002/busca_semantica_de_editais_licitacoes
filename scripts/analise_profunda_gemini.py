"""Análise profunda dos 100 editais de teste usando o Gemini como embedder,
em vez da Voyage (ADR-0001) — teste de viabilidade antes de mudar o pipeline
de produção.

Reaproveita a lógica de produção (analise_profunda.calcular_scores_aderencia,
Prioridade de seção) e os chunks já regenerados com o filtro Edital/TR em
dados/chunks/. Cache de embeddings próprio (não mistura com o cache Voyage).
"""
...
CACHE_CHUNKS_GEMINI = DADOS / ".embeddings" / "chunks_gemini.json"
SAIDA = DADOS / "analise_profunda_gemini_top100.json"
...
def main() -> None:
    ...
    embedder_de_chunks = criar_embedder_com_cache(embedder, CACHE_CHUNKS_GEMINI, MODELO_PADRAO)
